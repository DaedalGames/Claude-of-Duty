...
let failed = null;
try {
  await page.goto(`http://127.0.0.1:${PORT}/?capture=1&look=${encodeURIComponent(args.look ?? "realistic")}${args.grade ? `&grade=${encodeURIComponent(args.grade)}` : ""}${args.layers !== undefined ? `&layers=${encodeURIComponent(args.layers === true ? "" : args.layers)}` : ""}&prewarm=${args.prewarm ?? "1"}&q=${encodeURIComponent(args.q ?? "ultra")}&shot=${encodeURIComponent(SHOT)}`, {
